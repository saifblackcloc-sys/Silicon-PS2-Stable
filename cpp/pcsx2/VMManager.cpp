...
//#ifdef _M_X86 // TODO(Stenzek): Remove me once EE/VU/IOP recs are added.
    ...
    else
    {
        ...
        if (core_type == 2 || EmuConfig.Cpu.UseArm64Dynarec)
        {
             ...
             Cpu = &jitA64Cpu;
             EmuConfig.Speedhacks.EECycleRate = 2; // Slow down EE on ARM64 to prevent de-sync during FMV transitions
             EmuConfig.Speedhacks.EECycleSkip = 0; // Disable cycle skipping for stability
        }
        else
        {
