...
Result AudioStreamAAudio::open() {
    ...
    // Do not set INPUT capacity below 16384 because that prevents us from getting a FAST track
    // when using the Legacy data path.
    // If the app requests > 16384 then we allow it but we are less likely to get LowLatency.
